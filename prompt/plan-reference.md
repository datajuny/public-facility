...
## 디자인 (반드시 이대로)
- 배경 흰색, 텍스트 slate-900
- 선택 전 구: 채움 #94a3b8 (opacity 0.15), 테두리 #1e293b, weight 1.5
- 선택된 구:  채움 #0ea5e9 (opacity 0.35), 테두리 #0369a1, weight 2.5
- 카드: 흰 배경 · rounded-xl · shadow-sm · border-slate-200 · p-4
- 데스크톱(md+): 좌(지도) flex-[2] / 우(패널) flex-1
- 모바일: 위(지도) 60vh / 아래(패널) 40vh
...
당신이 **주도한 일** (CLAUDE.md에 정리된 내용)
- 이 앱의 핵심 개념(Facility · District) → 폴더와 파일 이름
- 디자인 수치(색 HEX, opacity, 레이아웃 비율)
