...
- **지도 위 플로팅 UI**
  * 공통: `bg-white/95 rounded-full shadow-lg ring-1 ring-slate-200/80 backdrop-blur-md`
  * z-index: `z-[1000]` (Leaflet 컨테이너 기본 z-index 0 아래로 가지 않게 `.leaflet-container { z-index: 0 }` 추가)
- **지도 타일**
...
3. web/src/lib/facilityMarker.ts 생성 — Leaflet divIcon 팩토리
   ...
   - 크기 24x24, 흰색 테두리 2px, 그림자
   - Leaflet 의 `L.divIcon({ html, className, iconSize })` 사용

4. web/src/components/CategoryFilter.tsx 생성 — 카테고리 토글 바
...
6. web/src/app/page.tsx 수정 — 리스트 "모드" 개념 도입
